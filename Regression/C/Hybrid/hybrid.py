...
target_column = 'G3'
X = data.drop(columns=["G1","G2",target_column])
...
def create_nn(input_dim):
    print(input_dim)
    model = Sequential([
      Dense(57, input_dim=input_dim, activation='sigmoid'),
...
nn.fit(X_train_processed, y_train, epochs=500, batch_size=32, verbose=0)

# Neural network predictions
nn_predictions = nn.predict(X_test_processed).ravel()

# ---------------------- Hybrid Model ----------------------
alpha = 0.9  # Adjust weight for Random Forest
